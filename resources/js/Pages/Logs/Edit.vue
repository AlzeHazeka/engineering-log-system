...
| FORM INITIAL VALUE
|--------------------------------------------------------------------------
| Pastikan description diisi dengan HTML lama
*/
const form = useForm({
    system_id: props.log.system_id,
    type: props.log.type,
    impact: props.log.impact,
    title: props.log.title,
    description: props.log.description || "",
    logged_at: props.log.logged_at.slice(0, 16),
});

const submit = () => {
    form.put(route("logs.update", props.log.id));
};
</script>
...
<template>
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold text-gray-800">Edit Log</h2>
        </template>
